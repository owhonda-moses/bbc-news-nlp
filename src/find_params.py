...
from src.train_baseline import NewsClassifierDataset, eval_model
...
N_TRIALS = 50
OUTPUT_DIR = './params'
SEED = None  # integer for reproducibility/None for better exploration
...
def objective(trial, train_df, val_dataset, unique_labels, device):
    ...
    l_r = np.logspace(np.log10(1e-5), np.log10(5e-4), 20)
    learning_rate = trial.suggest_categorical("learning_rate", l_r.tolist())
    weight_decay = trial.suggest_float('weight_decay', 0.0, 0.1, step=0.005)

    label2id = {label: i for i, label in enumerate(unique_labels)}
    id2label = {i: label for label, i in label2id.items()}

    # dataset creation per-trial
    ...
    for epoch in range(EPOCHS):
        ...
        for batch in train_loader:
            ...
            loss = loss_fct(outputs.logits, labels) # manually apply weighted loss
            loss.backward()
            optimizer.step()
...
def main():
    ...
    best_params['f1_score'] = study.best_value
